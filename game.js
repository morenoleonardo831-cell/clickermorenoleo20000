...
const CLOUD_API_BASE = window.CLOUD_API_BASE || "http://localhost:4000";
const CLOUD_TOKEN_KEY = "imperio_clicker_cloud_token_v1";
const CLOUD_EMAIL_KEY = "imperio_clicker_cloud_email_v1";
const ADMIN_API_TOKEN_KEY = "imperio_clicker_admin_token_v1";
const FARM_HECTARE_PRICE = 90000;
const FARM_YIELD_PCT = 0.0075;
const COMPANY_MARGIN_OPTIONS = [
  { label: "Conservador", mult: 0.8 },
  { label: "Equilibrado", mult: 0.9 },
  { label: "Agressivo", mult: 1 },
  { label: "Premium", mult: 1.1 }
];
...
const companies = [
  { id: "quiosque", nome: "Quiosque", setor: "Varejo", baseCost: 75000, baseIncome: 2000, owned: 0, level: 0, employees: 0, salaryTier: 1, marketFactor: 1, lastIncome: 0 },
  { id: "cafeteria", nome: "Cafeteria", setor: "Alimentos", baseCost: 225000, baseIncome: 5200, owned: 0, level: 0, employees: 0, salaryTier: 1, marketFactor: 1, lastIncome: 0 },
  { id: "mercado", nome: "Mini mercado", setor: "Varejo", baseCost: 525000, baseIncome: 11200, owned: 0, level: 0, employees: 0, salaryTier: 1, marketFactor: 1, lastIncome: 0 },
  { id: "academia", nome: "Academia", setor: "Servicos", baseCost: 900000, baseIncome: 18400, owned: 0, level: 0, employees: 0, salaryTier: 1, marketFactor: 1, lastIncome: 0 },
  { id: "agencia_marketing", nome: "Agencia de marketing", setor: "Digital", baseCost: 1350000, baseIncome: 26400, owned: 0, level: 0, employees: 0, salaryTier: 1, marketFactor: 1, lastIncome: 0 },
  { id: "startup", nome: "Startup", setor: "Tecnologia", baseCost: 2100000, baseIncome: 40000, owned: 0, level: 0, employees: 0, salaryTier: 1, marketFactor: 1, lastIncome: 0 },
  { id: "fabrica", nome: "Fabrica", setor: "Industria", baseCost: 4500000, baseIncome: 80000, owned: 0, level: 0, employees: 0, salaryTier: 1, marketFactor: 1, lastIncome: 0 },
  { id: "rede_hoteis", nome: "Rede de hoteis", setor: "Turismo", baseCost: 10500000, baseIncome: 168000, owned: 0, level: 0, employees: 0, salaryTier: 1, marketFactor: 1, lastIncome: 0 },
  { id: "fintech", nome: "Fintech", setor: "Financeiro", baseCost: 22500000, baseIncome: 336000, owned: 0, level: 0, employees: 0, salaryTier: 1, marketFactor: 1, lastIncome: 0 },
  { id: "holding", nome: "Holding global", setor: "Conglomerado", baseCost: 52500000, baseIncome: 720000, owned: 0, level: 0, employees: 0, salaryTier: 1, marketFactor: 1, lastIncome: 0 },
  { id: "ecommerce", nome: "E-commerce nacional", setor: "Digital", baseCost: 97500000, baseIncome: 1280000, owned: 0, level: 0, employees: 0, salaryTier: 1, marketFactor: 1, lastIncome: 0 },
  { id: "logistica", nome: "Operadora logistica", setor: "Industria", baseCost: 142500000, baseIncome: 1840000, owned: 0, level: 0, employees: 0, salaryTier: 1, marketFactor: 1, lastIncome: 0 },
  { id: "construtora", nome: "Construtora urbana", setor: "Industria", baseCost: 210000000, baseIncome: 2640000, owned: 0, level: 0, employees: 0, salaryTier: 1, marketFactor: 1, lastIncome: 0 },
  { id: "rede_hospitais", nome: "Rede de hospitais", setor: "Servicos", baseCost: 330000000, baseIncome: 4160000, owned: 0, level: 0, employees: 0, salaryTier: 1, marketFactor: 1, lastIncome: 0 },
  { id: "telecom", nome: "Operadora telecom", setor: "Tecnologia", baseCost: 480000000, baseIncome: 6080000, owned: 0, level: 0, employees: 0, salaryTier: 1, marketFactor: 1, lastIncome: 0 },
  { id: "energia", nome: "Geradora de energia", setor: "Industria", baseCost: 750000000, baseIncome: 9760000, owned: 0, level: 0, employees: 0, salaryTier: 1, marketFactor: 1, lastIncome: 0 },
  { id: "biotech", nome: "Biotech farmaceutica", setor: "Tecnologia", baseCost: 1170000000, baseIncome: 15200000, owned: 0, level: 0, employees: 0, salaryTier: 1, marketFactor: 1, lastIncome: 0 },
  { id: "banco_investimentos", nome: "Banco de investimentos", setor: "Financeiro", baseCost: 1800000000, baseIncome: 24000000, owned: 0, level: 0, employees: 0, salaryTier: 1, marketFactor: 1, lastIncome: 0 }
];
...
const el = {
  ...
  adminApiEmail: document.getElementById("adminApiEmail"),
  adminApiPassword: document.getElementById("adminApiPassword"),
  adminApiLoginBtn: document.getElementById("adminApiLoginBtn"),
  adminApiLogoutBtn: document.getElementById("adminApiLogoutBtn"),
  adminApiStatus: document.getElementById("adminApiStatus"),
  adminUserEmail: document.getElementById("adminUserEmail"),
  adminFetchUserBtn: document.getElementById("adminFetchUserBtn"),
  adminUserInfo: document.getElementById("adminUserInfo"),
  adminUserMoneyInput: document.getElementById("adminUserMoneyInput"),
  adminUserAddMoneyBtn: document.getElementById("adminUserAddMoneyBtn"),
  adminUserRemoveMoneyBtn: document.getElementById("adminUserRemoveMoneyBtn"),
  adminUserSetMoneyBtn: document.getElementById("adminUserSetMoneyBtn"),
  adminUserSaveText: document.getElementById("adminUserSaveText"),
  adminUserLoadSaveBtn: document.getElementById("adminUserLoadSaveBtn"),
  adminUserApplySaveBtn: document.getElementById("adminUserApplySaveBtn")
};
...
function companyPayroll(c) {
  ...
  const base = companyTotalIncome(c) * 0.27;
...
function monthlyOperationalCost() {
  const variable = Math.floor(totalCompanyIncome() * 0.21);
...
let adminApiSession = { token: "", userId: "" };
...
function setCloudToken(token, email = "") {
...
function setAdminApiToken(token) {
  if (token) localStorage.setItem(ADMIN_API_TOKEN_KEY, token);
...
function setCloudStatus(msg, type = "") {
  ...
  el.cloudStatus.className = `mini ${type}`.trim();
...
function renderCloudStatus() {
  ...
  if (token) {
    ...
    if (el.cloudEmail && email && !el.cloudEmail.value) el.cloudEmail.value = email;
    setCloudStatus(`Conectado${email ? `: ${email}` : ""}.`, "ok");
  } else {
    setCloudStatus("Conta online desconectada.", "warn");
  }
}

function setAdminApiStatus(msg, type = "") {
  if (!el.adminApiStatus) return;
  el.adminApiStatus.className = `mini ${type}`.trim();
  el.adminApiStatus.textContent = msg;
}

function renderAdminApiStatus() {
  if (!el.adminApiStatus) return;
  const token = getAdminApiToken();
  setAdminApiStatus(token ? "Admin online conectado." : "Admin online desconectado.", token ? "ok" : "warn");
}
...
async function apiRequest(path, opts = {}) {
  ...
  if (!res.ok) {
    const msg = data?.error || "Falha na API.";
    throw new Error(msg);
  }
  return data;
}
...
async function cloudRegister() {
  const email = String(el.cloudEmail?.value || "").trim().toLowerCase();
  const password = String(el.cloudPassword?.value || "");
  if (!email || password.length < 6) {
    setCloudStatus("Informe email e senha valida (min 6).", "warn");
    return;
  }
  try {
    const data = await apiRequest("/api/auth/register", { method: "POST", body: JSON.stringify({ email, password }) });
    setCloudToken(data.token, email);
    setCloudStatus("Conta criada e conectada.", "ok");
  } catch (err) {
    setCloudStatus(err.message, "bad");
  }
}
...
async function cloudSave() {
  const token = getCloudToken();
  if (!token) {
    setCloudStatus("Faca login antes de salvar.", "warn");
    return;
  }
  const payload = {
    data: serializeState(),
  ...
  try {
    ...
    setCloudStatus("Save enviado para a nuvem.", "ok");
  } catch (err) {
    setCloudStatus(err.message, "bad");
  }
}
...
async function cloudLoad() {
  const token = getCloudToken();
  if (!token) {
    setCloudStatus("Faca login antes de carregar.", "warn");
    return;
  }
  try {
    const data = await apiRequest("/api/save", { headers: { Authorization: `Bearer ${token}` } });
    applySave(data.data);
    render();
    setCloudStatus("Save carregado da nuvem.", "ok");
  } catch (err) {
    setCloudStatus(err.message, "bad");
  }
}

async function adminApiLogin() {
  const email = String(el.adminApiEmail?.value || "").trim().toLowerCase();
  const password = String(el.adminApiPassword?.value || "");
  if (!email || !password) {
    setAdminApiStatus("Informe email e senha.", "warn");
    return;
  }
  try {
    const data = await apiRequest("/api/auth/login", { method: "POST", body: JSON.stringify({ email, password }) });
    setAdminApiToken(data.token);
    setAdminApiStatus("Admin conectado.", "ok");
  } catch (err) {
    setAdminApiStatus(err.message, "bad");
  }
}

async function adminApiFetchUser() {
  const token = getAdminApiToken();
  const email = String(el.adminUserEmail?.value || "").trim().toLowerCase();
  if (!token) {
    setAdminApiStatus("Faca login admin.", "warn");
    return;
  }
  if (!email) {
    setAdminApiStatus("Informe o email do jogador.", "warn");
    return;
  }
  try {
    const data = await apiRequest(`/api/admin/users?search=${encodeURIComponent(email)}`, {
      headers: { Authorization: `Bearer ${token}` }
    });
    const match = (data.users || []).find((u) => String(u.email).toLowerCase() === email) || data.users?.[0];
    if (!match) {
      adminSelectedUserId = "";
      if (el.adminUserInfo) el.adminUserInfo.textContent = "Jogador nao encontrado.";
      return;
    }
    adminSelectedUserId = match.id;
    if (el.adminUserInfo) {
      const save = match.save;
      el.adminUserInfo.textContent = save
        ? `ID ${match.id} | ${match.email} | Patrimonio ${formatMoney(save.patrimonio)} | Dinheiro ${formatMoney(save.money)} | Atualizado ${new Date(save.updatedAt).toLocaleString("pt-BR")}`
        : `ID ${match.id} | ${match.email} | Sem save registrado.`;
    }
  } catch (err) {
    setAdminApiStatus(err.message, "bad");
  }
...
async function adminApiAdjustMoney(mode) {
  const token = getAdminApiToken();
  const amount = Math.floor(safeNumber(el.adminUserMoneyInput?.value, 0));
  if (!token) {
    setAdminApiStatus("Faca login admin.", "warn");
    return;
  }
  if (!adminSelectedUserId) {
    setAdminApiStatus("Selecione um jogador.", "warn");
    return;
  }
  if (!amount || amount <= 0) {
    setAdminApiStatus("Informe um valor valido.", "warn");
    return;
  }
  try {
    await apiRequest(`/api/admin/users/${adminSelectedUserId}/money`, {
      method: "PATCH",
      headers: { Authorization: `Bearer ${token}` },
      body: JSON.stringify({ mode, amount })
    });
    await adminApiFetchUser();
    setAdminApiStatus("Saldo atualizado.", "ok");
  } catch (err) {
    setAdminApiStatus(err.message, "bad");
  }
}

async function adminApiLoadSave() {
  const token = getAdminApiToken();
  if (!token) {
    setAdminApiStatus("Faca login admin.", "warn");
    return;
  ...
  try {
    const data = await apiRequest(`/api/admin/users/${adminSelectedUserId}`, {
      headers: { Authorization: `Bearer ${token}` }
    });
    if (el.adminUserSaveText) {
      el.adminUserSaveText.value = data.save ? JSON.stringify(data.save.data, null, 2) : "";
    }
    setAdminApiStatus("Save carregado.", "ok");
  } catch (err) {
    setAdminApiStatus(err.message, "bad");
  }
}

async function adminApiApplySave() {
  const token = getAdminApiToken();
  if (!token) {
    setAdminApiStatus("Faca login admin.", "warn");
    return;
  ...
  let data;
  try {
    data = JSON.parse(String(el.adminUserSaveText?.value || ""));
  } catch {
    setAdminApiStatus("JSON invalido.", "bad");
    return;
  }
  try {
    await apiRequest(`/api/admin/users/${adminSelectedUserId}/save`, {
      method: "PUT",
      headers: { Authorization: `Bearer ${token}` },
      body: JSON.stringify({ data })
    });
    await adminApiFetchUser();
    setAdminApiStatus("Save aplicado.", "ok");
  } catch (err) {
    setAdminApiStatus(err.message, "bad");
  }
}
...
function taxRateFor(avgMonthly) {
  ...
  if (income <= 10000) return 0.14;
  if (income <= 50000) return 0.22;
  if (income <= 150000) return 0.31;
  if (income <= 400000) return 0.4;
  if (income <= 900000) return 0.5;
  return 0.58;
}
...
function renderRanking() {
  ...
  const playerName = (state.player.nome || "Jogador").trim();
  const playerPhone = (state.player.telefone || "").replace(/\D/g, "");
  const liveEntry = {
    name: playerName,
    phone: playerPhone || "-",
    patrimonio: getPatrimonio(),
  ...
  if (playerPhone) {
    const idx = combined.findIndex((x) => String(x.phone) === playerPhone);
    if (idx >= 0) combined[idx] = { ...combined[idx], ...liveEntry };
    else combined.push(liveEntry);
  } else if (combined.length === 0) {
    combined.push(liveEntry);
  }
  ...
  combined
    ...
    .forEach((r, i) => {
      ...
      const label = r.live ? `${name} (voce)` : name;
      const row = document.createElement("div");
      row.className = "rank-item";
      row.innerHTML = `
        <div class="rank-head"><span>#${i + 1} ${label}</span><span>${formatMoney(Number(r.patrimonio || 0))}</span></div>
        <div class="rank-meta">Tel: ${phone} | Nivel ${Number(r.level || 1)} | Prestigio ${Number(r.prestige || 0)} | Dinheiro ${formatMoney(Number(r.money || 0))}</div>
...
function renderWorldRanking() {
  ...
  const showPlayer = playerRank > 5 && playerRank <= 6;
  const displayList = showPlayer ? top5.concat(roster[playerRank - 1]) : top5;
  ...
  displayList.forEach((r, i) => {
    ...
    row.innerHTML = `
      <div class="rank-head"><span>#${i + 1} ${label}</span><span class="rank-value">${formatMoney(r.wealthBrl)}</span></div>
...
function buildAnnualCloseSummary() {
  ...
  const patrimonio = Math.max(0, getPatrimonio());
  let wealthRate = 0;
  if (patrimonio > 1000000000) wealthRate = 0.015;
  else if (patrimonio > 200000000) wealthRate = 0.01;
  else if (patrimonio > 50000000) wealthRate = 0.005;
  const yearlyWealthTax = Math.floor(patrimonio * wealthRate);
  ...
  return {
    ...
    debtInterest,
    debtDue,
    totalDue: yearlyTax + yearlyWealthTax + yearlyIpva + debtDue
  };
}
...
function updateTaxOverlay() {
  ...
  const patrimonio = Math.max(0, safeNumber(summary.patrimonio, 0));
  const wealthRate = clamp(safeNumber(summary.wealthRate, 0), 0, 1);
  const yearlyWealthTax = Math.max(0, safeNumber(summary.yearlyWealthTax, 0));
    ...
    el.taxDetails.innerHTML = `
      <div class="tax-grid">
        <div class="tax-block">
          ...
          <div class="tax-row"><span>Base de patrimonio</span><strong>${formatMoney(patrimonio)}</strong></div>
          <div class="tax-row"><span>Aliquota grandes fortunas</span><strong>${(wealthRate * 100).toFixed(2)}%</strong></div>
        </div>
        <div class="tax-block">
          ...
          <div class="tax-row"><span>Imposto grandes fortunas</span><strong>${formatMoney(yearlyWealthTax)}</strong></div>
  ...
  if (data.tax && typeof data.tax === "object") {
    const pending = data.tax.pendingYearClose && typeof data.tax.pendingYearClose === "object"
      ? {
        ...
        clickYearRevenue: Math.max(0, safeNumber(data.tax.pendingYearClose.clickYearRevenue, 0)),
        ...
        patrimonio: Math.max(0, safeNumber(data.tax.pendingYearClose.patrimonio, 0)),
        wealthRate: clamp(safeNumber(data.tax.pendingYearClose.wealthRate, 0), 0, 1),
        yearlyWealthTax: Math.max(0, safeNumber(data.tax.pendingYearClose.yearlyWealthTax, 0)),
...
if (el.cloudRegisterBtn) el.cloudRegisterBtn.addEventListener("click", cloudRegister);
if (el.cloudLoginBtn) el.cloudLoginBtn.addEventListener("click", cloudLogin);
if (el.cloudSaveBtn) el.cloudSaveBtn.addEventListener("click", cloudSave);
if (el.cloudLoadBtn) el.cloudLoadBtn.addEventListener("click", cloudLoad);
if (el.cloudLogoutBtn) el.cloudLogoutBtn.addEventListener("click", () => {
  clearCloudToken();
  setCloudStatus("Conta desconectada.", "warn");
});
...
el.loanSmallBtn.addEventListener("click", () => requestLoan(25000, 12, 0.22, "Emprestimo pequeno"));
el.loanMediumBtn.addEventListener("click", () => requestLoan(90000, 18, 0.3, "Emprestimo medio"));
el.loanLargeBtn.addEventListener("click", () => requestLoan(260000, 24, 0.38, "Emprestimo grande"));
...
if (el.adminApiLoginBtn) el.adminApiLoginBtn.addEventListener("click", adminApiLogin);
if (el.adminApiLogoutBtn) el.adminApiLogoutBtn.addEventListener("click", () => {
  clearAdminApiToken();
  setAdminApiStatus("Admin online desconectado.", "warn");
});
if (el.adminFetchUserBtn) el.adminFetchUserBtn.addEventListener("click", adminApiFetchUser);
if (el.adminUserAddMoneyBtn) el.adminUserAddMoneyBtn.addEventListener("click", () => adminApiAdjustMoney("add"));
if (el.adminUserRemoveMoneyBtn) el.adminUserRemoveMoneyBtn.addEventListener("click", () => adminApiAdjustMoney("remove"));
if (el.adminUserSetMoneyBtn) el.adminUserSetMoneyBtn.addEventListener("click", () => adminApiAdjustMoney("set"));
if (el.adminUserLoadSaveBtn) el.adminUserLoadSaveBtn.addEventListener("click", adminApiLoadSave);
if (el.adminUserApplySaveBtn) el.adminUserApplySaveBtn.addEventListener("click", adminApiApplySave);
